...
我们借助在文档 [HTTP 负载均衡](/features/http-load-balancer/) 中的负载均衡器配置，修改服务的端点列表为我们的后端服务地址 `127.0.0.1:8081`；同时将该后端服务的路由路径设置为 `/sample`；最后根据 [HTTP 插件链配置](/reference/plugin/#http-路由)，将 `filter/url-rewrite.js` 插件配置在相应的配置。
